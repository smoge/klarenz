...
inline sample_type phasorComputeSample(internal_type &phase,
                                       const internal_type &phase_inc,
...
constexpr void phasorPerform(const sample_type *in, sample_type *out,
                             internal_type &phase, const internal_type freq_factor,
                             const internal_type wrap) {
    if constexpr (n > 0) {
...
template <unsigned int n, typename sample_type, typename internal_type>
constexpr void phasorPerform(sample_type *out, internal_type &phase,
                             const internal_type phase_inc, const internal_type wrap) {
    if constexpr (n > 0) {
        *out = phasorComputeSample<sample_type, internal_type>(phase, phase_inc, wrap);
...
class Phasor {
    static_assert(std::is_floating_point_v<sample_type>,
                  "sample_type must be floating point");
    static_assert(std::is_floating_point_v<internal_type>,
                  "internal_type must be floating point");
  ...
  public:
   ...
   inline void perform(input_buffer_type const &in, output_buffer_type &out, unsigned int n) {
    ...
    template <typename input_buffer_type, typename output_buffer_type>
    inline void perform8(input_buffer_type const &in, output_buffer_type &out,
                         unsigned int n) {
        const sample_type *inSample = in.data();
        sample_type *outSample = out.data();
    ...
    void setPhase(internal_type const &new_phase) { phase_ = new_phase; }
  ...
  protected:
    inline static internal_type wrap() { return internal_type(wrap_arg); }

    static inline sample_type computeSample(internal_type &phase,
                                            const internal_type phase_inc) {
        return detail::phasorComputeSample<sample_type, internal_type>(phase, phase_inc,
                                                                       wrap());
    }
